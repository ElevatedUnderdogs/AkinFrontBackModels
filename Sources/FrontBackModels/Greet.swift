...
public extension Greet {
    ...
    var otherUserContents: OtherUserContents {
        OtherUserContents(
            otherUserName: otherUser.name,
            greetingMethod: method.rawValue,
            travelStatusText: otherUserTravelStatusText
        )
    }
    ...
    func viewContents(now: Date = .init()) -> GreetViewContents? {
        ...
        // if the highest is 3 but there are fewer than 3 elements we need to fetch the rest.
        if Set(sortedEvents.map(\.serverSequenceNumber)).count < last.serverSequenceNumber {
            // fetch more.
            return nil
        }

        if sortedEvents.contains(where: { $0.action.isRated && $0.actorUserID == thisUserID }) {
            return .showInHistory
        }

        if sortedEvents.contains(where: { $0.action == .confirmedMet && $0.actorUserID == thisUserID }) {
            return .giveRating
        }


        // This user's rejections first.
        if sortedEvents.contains(
            where: {
                ($0.action == .dismissGreet || $0.action == .tappedRedVoipReject) && $0.actorUserID == thisUserID }
        ) {
            return .rejected(.thisUserDismissed)
        }

        if sortedEvents.contains(
            where: {
                ($0.action == .dismissGreet || $0.action == .tappedRedVoipReject) && $0.actorUserID == otherUser.id }
        ) {
            return .rejected(.otherUserDismissed)
        }

        let theirTravelUpdates: [GreetEvent] = sortedEvents
            .filter { $0.actorUserID == otherUser.id && $0.action.isTravelTimeUpdate }
        let thisTravelUpdates: [GreetEvent] = sortedEvents
            .filter { $0.actorUserID == thisUserID && $0.action.isTravelTimeUpdate }

        if let travelTime = theirTravelUpdates.last?.action.travelTime,
           travelTime > (otherMinutesAway + wrongWayThreshold) {
            return .rejected(.theyWrongWay(travelTime: travelTime))
        }

        if let travelTime = thisTravelUpdates.last?.action.travelTime,
           travelTime > (minutesAway + wrongWayThreshold) {
            return .rejected(.thisWrongWay(travelTime: travelTime))
        }

        // if a user is this many minutes away from the venue, we can treat them as
        // essentially arrived at the venue.
        let arrivedMinutes: Int = 5
        let minutesInInterval: Double = 10 * 60
        if let thisShortestTravelTimeEvent: GreetEvent = thisTravelUpdates.min(by: { $0.action.travelTime ?? .max < $1.action.travelTime ?? .max }),
            let travelTime = thisShortestTravelTimeEvent.action.travelTime {
            if travelTime > arrivedMinutes { // they haven't arrived.
                // no progress made in the minutesInInterval
                if let firstShortTravelTime = thisTravelUpdates
                    .sorted(by: { $0.serverDate < $1.serverDate })
                    .first(where: { $0.action.travelTime == travelTime }),
                   abs(firstShortTravelTime.serverDate.timeIntervalSince(now)) > minutesInInterval {
                    return .rejected(.thisSlowProgress(travelTime: travelTime))
                }
            }
        }
        if let theirShortestTravelTimeEvent: GreetEvent = theirTravelUpdates.min(by: { $0.action.travelTime ?? Int.max < $1.action.travelTime ?? Int.max }),
           let travelTime = theirShortestTravelTimeEvent.action.travelTime {
            if travelTime > arrivedMinutes { // they haven't arrived.
                // no progress made in the minutesInInterval
                if let firstShortTravelTime = theirTravelUpdates
                    .sorted(by: { $0.serverDate < $1.serverDate })
                    .first(where: { $0.action.travelTime == travelTime }),
                abs(firstShortTravelTime.serverDate.timeIntervalSince(now)) > minutesInInterval {
                    return .rejected(.theySlowProgress(travelTime: travelTime))
                }
            }
        }

        let agreeEvents: [GreetEvent] = sortedEvents.filter(\.action.isAgreeToMeet)
        let thisAgreedActions: [GreetAction] = agreeEvents.filter { $0.actorUserID == thisUserID }.map(\.action)
        let thisAgreeTimes: [Int] = thisAgreedActions.compactMap(\.agreeToTime)
        let theyAgreeActions: [GreetAction] = agreeEvents.filter { $0.actorUserID == otherUser.id }.map(\.action)
        let theyAgreeTimes: [Int] = theyAgreeActions.compactMap(\.agreeToTime)
        let commonAgreements = Set(thisAgreedActions).intersection(Set(theyAgreeActions))

        // || $0.action == .closeApp
        let hasAgreements: Bool = thisAgreeTimes.contains(where: { theyAgreeTimes.contains($0) })
        // if both users are enroute or agreed, they may close the app for the purpose of mapping.
        if sortedEvents.contains(where: { $0.action == .closeApp && $0.actorUserID == thisUserID }) && !hasAgreements {
            return .rejected(.thisUserDismissed)
        } else if sortedEvents.contains(where: { $0.action == .closeApp && $0.actorUserID == otherUser.id }) && !hasAgreements {
            return .rejected(.otherUserDismissed)
        }

        assert(commonAgreements.count <= 1)


        if let agreeToTimeInMinutes = commonAgreements.first?.agreeToTime,
           let dateOfLastAgreement: Date = agreeEvents.last(where: { $0.action == commonAgreements.first })?.serverDate {

            // if the latest location update for each sum is less than 5 or the threshold, then we ask if they are confirmed met// they are close dialogue.
            let travelTimeEvents: [GreetEvent] = sortedEvents.filter(\.action.isTravelTimeUpdate)
            let thisLastTravelTimeUpdate: Int = travelTimeEvents
                .last(where: { $0.actorUserID == thisUserID })?
                .action
                .travelTime ?? minutesAway

            let otherLastTravelTimeUpdate: Int = travelTimeEvents
                .last(where: { $0.actorUserID == otherUser.id })?
                .action
                .travelTime ?? otherMinutesAway
            let youTwoAreClose: Bool = (thisLastTravelTimeUpdate + otherLastTravelTimeUpdate) <= 5
            let theyConfirmedMet: Bool = sortedEvents.filter { $0.actorUserID == otherUser.id }.contains(where: { $0.action == .confirmedMet })
            return .enroute(
                EnrouteContents(
                    profilePicURL: otherProfilePic,
                    openers: openers,
                    address: venue.address,
                    percentTravelled: Double(otherMinutesAway - otherLastTravelTimeUpdate) / Double(otherMinutesAway),
                    instructionContents: InstructionCellContents(
                        venueNaeme: venue.name,
                        targetMeetTime: Calendar.current.date(
                            byAdding: .minute,
                            value: [minutesAway, otherMinutesAway].max()! + agreeToTimeInMinutes,
                            to: dateOfLastAgreement
                        )!
                    ),
                    otherContents: OtherUserContents(
                        otherUserName: otherUser.name,
                        greetingMethod: method.rawValue,
                        travelStatusText: otherUserTravelStatusText
                    ),
                    alternateRequestCellModel: theyConfirmedMet ? .theySaidTheyMet(otherName: otherUser.name) :
                        youTwoAreClose ? .youAreClose(otherName: otherUser.name) :
                            nil
                )
            )
        }

        let now: MeetTimeState = thisAgreedActions.contains(.agreedToMeet(0)) ? .waiting : .show
        let in30: MeetTimeState = sortedEvents.contains(where: {$0.action == .rejectTime(30) }) ? .rejectedHidden :
        thisAgreedActions.contains(.agreedToMeet(30)) ? .waiting : .show
        let is30Rejected: Bool = sortedEvents.filter(\.action.isRejectMeet).map(\.action).contains(.rejectTime(30))
        let showProposal: Bool = theyAgreeActions.contains(.agreedToMeet(30)) &&
        !(thisAgreedActions.contains(.agreedToMeet(30)) || is30Rejected)
        ...
        return .negotiation(
            StartViewContents(
                ...
                meetDecisionContents: MeetDecisionCellContents(
                    venueName: venue.name,
                    proposalState: showProposal ? .alternateProposal(.theyProposed(minutes: 30)) :
                            .select(now: now, in30: in30)
                )
            )
        )
    }
