...
public enum GreetActionEvent: Codable {
    ...
    case tappedRedVoipReject(id: UUID)
    ...
    public var channel: GreetActionChannel {
        switch self {
        case .pushQueued(let ch, _), .pushSent(let ch, _), .pushFailed(let ch): return ch
        case .deliveryConfirmed(let maybeCh, _): return maybeCh
        case .fallbackTriggered(let to): return to
        case .webSocketSent: return .websocket
        case .webSocketFailed, .rateLimited,
             ...
             .greetCreated, .greetCanceled, .greetExpired, .settingsUpdated, .pushProviderAccepted:
    ...
    /// Provider message identifier (e.g., Apple Push Notification service `apns-id`) if applicable.
    public var providerMessageID: UUID? {
        switch self {
        case .pushQueued(_, let id), .pushSent(_, let id): return id
        case .pushProviderAccepted(let id): return id
        case .deliveryConfirmed(_, let id): return id
        case .pushFailed, .webSocketSent, .webSocketFailed,
             .fallbackTriggered, .rateLimited,
             .userTappedMeet, .userTappedCancel, .userTappedDismiss,
             .userSelectedLater30m, .userAgreedLater30m, .userRejectedLater30m,
             .greetCreated, .greetCanceled, .greetExpired, .settingsUpdated:
            return nil
        case .tappedRedVoipReject(let id):
            return id
        }
    }

    /// Default actor kind to record for this event.
    /// Override when you have stronger context (e.g., a known user action).
    public var actorKindDefault: GreetActionActorKind {
        ...
        // Device handled something automatically; treat as system by default.
        // If you tie this to an explicit tap, pass `.user` explicitly when creating the action.
        case .deliveryConfirmed:
            return .system

        // Explicit user intents
        case .userTappedMeet, .userTappedCancel, .userTappedDismiss,
             .userSelectedLater30m, .userAgreedLater30m, .userRejectedLater30m, .tappedRedVoipReject:
            return .user

        // State/system
        case .greetExpired:
            return .system
        case .greetCreated, .greetCanceled, .settingsUpdated:
            return .server
        }
    }
}
